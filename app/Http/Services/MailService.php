...
class MailService
{
	...
	{
		for ($i = 1; $i < 3; $i++) {
			if (isset($data["rodo$i"]) && $data["rodo$i"])
				$data["rodo$i"] = 'Zaakceptowane';
			else
				$data["rodo$i"] = 'Niezaakceptowane';
		}
	...
	public static function getTemplate(array $data): Mailable
	{
		if (isset($data['answer']))
			return new AnswerForm($data);
		else if (isset($data['newsletter']))
			return new NewsletterForm($data);
		else
	...
	public static function saveData(Request $request)
	{
		if ($request->isMethod("POST")) {
			...
			if (!RecaptchaHelper::validate($request->input('response')))
				return ResponseHelper::invalidRecaptcha();
		}
