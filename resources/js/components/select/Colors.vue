  ...
  >
    <template v-slot:prepend-item>
      ...
      <v-list-item>
        ...
        <v-btn color="primary" @click="addColor" :disabled="newColor == ''" text
          >Wyślij</v-btn
        >
      </v-list-item>
...
export default {
  ...
  methods: {
    ...
    async getColors() {
      await axios
        .get("/api/shop_item_colors/get_all")
        .then((res) => {
          this.colors = res.data;
          if (this.selectedColor != 0)
            this.color = this.colors.find(
              (color) => this.selectedColor == color.id
            );
        })
        .catch((err) => {
    ...
    searchColor() {
      ...
      for (let color of this.colors)
        color.title.toLowerCase().includes(this.search.toLowerCase())
    ...
    async addColor() {
      ...
      await axios
        .post(`/api/shop_item_colors/add`, { title: this.newColor })
        .then((res) => {
          this.newColor = "";
    ...
    async edit(item) {
      ...
      this.$store.commit("loading", true);
      await axios
        .put(`/api/shop_item_colors/edit`, item)
        .then((res) => {
          this.$store.commit("loading", false);
          this.editing = false;
