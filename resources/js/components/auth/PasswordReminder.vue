<template>
  <div>
    <v-dialog
      @input="(v) => v || closePasswordReminder()"
      class="position-relative"
      v-model="openPasswordReminder"
      persistent
    >
      <v-card
        class="login-card login-bg"
        raised
        :style="`background-image: linear-gradient(to right top, rgb(44 60 49 / 70%), hsl(0 0% 0% / 0.9)), url(${url(
          authDescriptions.bg_password_reminder_photo
        )})`"
      >
        ...
        <v-form
          ...
          ref="form"
          class="position-relative login-form"
        >
          <h1 class="about-title font-weight-bold text-center text-white mt-0">
            {{ authDescriptions.password_reminder_title }}
          </h1>
          <v-text-field
            dark
            v-model="auth.email"
            :rules="[required, email]"
            :label="authDescriptions.email"
            required
          ></v-text-field>
        </v-form>
        <v-btn
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      axios
        .post("/api/users/password_reminder", this.auth)
        .then((res) => {
