  ...
  <v-stepper v-model="e1">
    <v-stepper-header>
      <v-stepper-step :complete="e1 > 1" step="1">{{
        cartDescriptions.step_1
      }}</v-stepper-step>
      <v-divider></v-divider>
      <v-stepper-step :complete="e1 > 2" step="2">{{
        cartDescriptions.step_2
      }}</v-stepper-step>
      <v-divider></v-divider>
      <v-stepper-step :complete="e1 > 3" step="3">{{
        cartDescriptions.step_3
      }}</v-stepper-step>
      <v-divider></v-divider>
    ...
    <v-stepper-items>
      <v-stepper-content step="1">
        ...
        <v-btn :disabled="cart.length == 0" color="primary" @click="e1 = 2">{{
          cartDescriptions.next
        }}</v-btn>
      </v-stepper-content>
      ...
      <v-stepper-content step="2">
        <div class="mb-12">
          <ShippingDetails
            ...
            @shippingDetails="shippingDetails = $event"
            @e1="e1 = $event"
            :submit="shippingDetailsSubmit"
            @valid="shippingDetailsValid = $event"
          />
        </div>

        <v-btn
          :disabled="!shippingDetailsValid"
          color="primary"
          @click="shippingDetailsValidate"
          >{{ cartDescriptions.next }}</v-btn
        >
      ...
      <v-stepper-content step="3">
        <div class="mb-12">
          <Payments
            ...
            @e1="e1 = $event"
            :submit="paymentsSubmit"
            @valid="paymentsValid = $event"
          />
        </div>
        <v-btn
          :disabled="!paymentsValid"
          color="primary"
          @click="paymentsValidate"
          >{{ cartDescriptions.next }}</v-btn
        >
      ...
      <v-stepper-content step="4">
        ...
        <v-checkbox v-model="regulations">
          <span slot="label">
            {{ authDescriptions.shop_regulations }}
            <a
              @click.stop=""
              class="register-checkbox-link"
              download
              :href="url($store.getters.settings.shop_regulations)"
            >
              <v-btn class="ml-2" color="primary" small outlined>{{
                authDescriptions.download
              }}</v-btn>
            </a>
          </span>
        </v-checkbox>
        <v-btn :disabled="!regulations" color="primary" @click="realize">{{
          cartDescriptions.realize
        }}</v-btn>
  ...
  methods: {
    ...
    setCart(data) {
      for (let res of data) {
        ...
        if (res.delete != undefined || res.amount == 0)
          cart.splice(
            cart.indexOf(
              cart.find(
                (product) =>
                  JSON.stringify(res.product) == JSON.stringify(product)
              )
            ),
            1
          );
          ...
          cart.find(
            (product) => JSON.stringify(product) == JSON.stringify(res.product)
          ).amount = res.amount;
    ...
    getAuthDescriptions() {
      axios
        .get("/api/auth_descriptions/get_one/1")
        .then((res) => (this.authDescriptions = res.data));
    },
    realize() {
      if (!confirm(this.shopDescriptions.order_confirm) || !this.regulations)
        return;

      this.$store.commit("loading", true);
      axios
        .post("/api/shop_orders/add", this.cartData)
        .then((res) => {
          ...
          if (res.data.error != undefined) {
            ...
            this.$store.commit(
              "toast",
              this.$store.getters.snackbarAlerts.order_success
            );
